...
- If is didn't installed, install it from **https://www.wireshark.org/download.html**.
...
- Double-click on it to **start the Capture** button (blue shark fin icon).

### 3. Generate Network Traffic
- Opened multiple websites (Google, YouTube, Wikipedia).
- Run *ping google.com* in the terminal.
- Sent a test email to generate SMTP/IMAP traffic.

### 4. Stop Capture
- Captured for around **1 minute**.
- Stopped the capture and saved it as **Result Sample.pcap**.
...
- **Total Packets Captured: 8930

## Screenshots

- **Protocols appear in packet.png.** - After the capture, the network what area the protocols are in.
- **Dashboard of Wireshark.png** - Dashboard of Wireshark and selecting the network to capture and analyze the network traffic.
- **Capture the network .png.** - Capture the network traffic with the selection of the network interference.
- **Result Sample. pcapng**—The result sample of the network traffic after capture; from the result sample we can analyze the protocols, etc.
